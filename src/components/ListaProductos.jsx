...
function ListaProductos({ productos }) {
 ...
 function buscar(e){ 
    const q=e.target.value.toLowerCase().trim(); document.querySelectorAll('.producto').forEach(card=>{ card.style.display=card.dataset.nombre.includes(q)?'':'none'; }); }
 ...
 return <section id="productos" className="seccion"><div className="contenedor">
  ...
  <div className="catalogo-herramientas">
    <label className="buscador">🔎 <input type="search" placeholder="Buscar producto..." onChange={buscar}/>
    </label><div className="filtros">{categorias.map(c=><button key={c} className={`filtro ${c==='Todos'?'activo':''}`} data-cat={c} onClick={()=>filtrar(c)}>{c}</button>)}</div></div>
  <div className="grid-productos">{productos.map(p=><Producto key={p.id} {...p}/>)}</div>
  <div id="aviso" className="aviso" aria-live="polite"></div>
  <dialog id="modal-producto" className="modal">
    <button className="cerrar" onClick={cerrarModal} aria-label="Cerrar">
        ×</button><img id="modal-img" src="/img/productos.svg" alt="Producto"/>
        <span id="modal-cat" className="categoria-mini"></span>
        <h3 id="modal-nombre"></h3><p id="modal-desc">
            </p><strong id="modal-precio">
                </strong></dialog>
 </div></section>;
}
